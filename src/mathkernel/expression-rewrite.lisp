...
Returns two values:
  1) ENV alist on success (possibly NIL if no wildcards were bound),
  2) SUCCESS-P (T if the match succeeded, NIL otherwise).

PATTERN and EXPR are sexprs.

ENV maps wildcard keys (keywords) to bound sexprs.

Supported wildcards:
  ?x      -> single subtree
  ?rest*  -> sequence of arguments in a list (possibly empty)."
  (labels
      ((bind-wildcard (key value env)
         "Bind KEY to VALUE in ENV, or check consistency if KEY already bound.
Returns the new ENV on success, or NIL on failure."
         (let ((cell (assoc key env)))
           (cond
             ((null cell)
              (acons key value env))
       ...
       (match-list (pats exprs env)
         "Match list of patterns PATS against list of expressions EXPRS under ENV.
Returns (values env success-p)."
         (cond
           ;; both empty → success
           ((and (null pats) (null exprs))
            (values env t))

           ;; pattern exhausted but exprs remain → fail
           ((null pats)
            (values nil nil))

           ;; pattern not empty, exprs empty
           ((null exprs)
            ;; Only possible success: first pat is a seq wildcard that can
            ;; bind to empty, and the rest of pats must also match empty exprs.
            (if (sequence-wildcard-symbol-p (car pats))
                (let* ((sw   (car pats))
                       (key  (wildcard-base-name sw))
                       (env1 (bind-wildcard key '() env)))
                  (if env1
                      (match-list (cdr pats) '() env1)
                      (values nil nil)))
                (values nil nil)))

           ;; first pattern element is a sequence wildcard
           ((sequence-wildcard-symbol-p (car pats))
            (let* ((sw      (car pats))
                   (key     (wildcard-base-name sw))
                   (restpat (cdr pats))
                   (n       (length exprs)))
              ;; Try all splits: exprs = prefix ++ suffix,
              ;; binding prefix (possibly empty) to the seq wildcard.
              (loop for k from 0 to n do
                (let* ((prefix (subseq exprs 0 k))
                       (suffix (subseq exprs k))
                       (env1   (bind-wildcard key prefix env)))
                  (when env1
                    (multiple-value-bind (env2 ok2)
                        (match-list restpat suffix env1)
                      (when ok2
                        (return (values env2 t))))))
                    finally (return (values nil nil)))))

           ;; normal element: match head, then recurse on tail
           (t
            (multiple-value-bind (env1 ok1)
                (match-pattern (car pats) (car exprs) env)
              (if (not ok1)
                  (values nil nil)
                  (match-list (cdr pats) (cdr exprs) env1))))))
       )                                ; end labels
    (cond
      ;; wildcard atom (?x or ?rest*)
      ((wildcard-symbol-p pattern)
       (let* ((key  (wildcard-base-name pattern))
              (env2 (bind-wildcard key expr env)))
         (if env2
             (values env2 t)
             (values nil nil))))
      ...
      ((consp pattern)
       (if (not (consp expr))
           (values nil nil)
           ;; match heads, then tails (lists handled by MATCH-LIST)
           (multiple-value-bind (env1 ok1)
               (match-pattern (car pattern) (car expr) env)
             (if (not ok1)
                 (values nil nil)
...
Returns two values:
  ...
  (labels
      ((bind-wildcard (key value env)
         (let ((cell (assoc key env)))
           (cond
             ((null cell) (acons key value env))
             ((equal (cdr cell) value) env)
             (t nil))))
       ...
       (match-list (pats exprs env)
         (cond
           ...
           ;; exprs exhausted but pattern remains → fail
           ((null exprs)
            ;; only allowed if the remaining pattern is a single seq wildcard
            (if (and (null (cdr pats))
                     (sequence-wildcard-symbol-p (car pats)))
                (let* ((w-sym (car pats))
                       (key   (wildcard-base-name w-sym))
                       (env2  (bind-wildcard key '() env)))
                  (if env2
                      (values env2 t)
                      (values nil nil)))
                (values nil nil)))

           ;; special case: pattern is a single sequence wildcard → bind ALL exprs
           ((and (null (cdr pats))
                 (sequence-wildcard-symbol-p (car pats)))
            (let* ((w-sym (car pats))
                   (key   (wildcard-base-name w-sym))
                   (env2  (bind-wildcard key exprs env)))
              (if env2
                  (values env2 t)
                  (values nil nil))))

           ;; prefix sequence wildcard: (?prefix* p1 p2 ...)
           ((and (sequence-wildcard-symbol-p (car pats))
                 (cdr pats))
            (let* ((seq-sym   (car pats))
                   (key       (wildcard-base-name seq-sym))
                   (restpats  (cdr pats))
                   (n-exprs   (length exprs)))
              ;; Try all possible splits exprs = prefix ++ tail,
              ;; binding prefix to ?prefix* and matching restpats to tail.
              (loop for k from 0 to n-exprs do
                (let* ((prefix (subseq exprs 0 k))
                       (tail   (subseq exprs k))
                       (env1   (bind-wildcard key prefix env)))
                  (when env1
                    (multiple-value-bind (env2 success2)
                        (match-list restpats tail env1)
                      (when success2
                        (return (values env2 t))))))
                    finally (return (values nil nil)))))

           ;; suffix sequence wildcard in last position
           ((and (null (cdr pats))
                 (sequence-wildcard-symbol-p (car pats)))
            ;; already handled above; this clause kept for clarity
            (let* ((w-sym (car pats))
                   (key   (wildcard-base-name w-sym))
                   (env2  (bind-wildcard key exprs env)))
              (if env2
           ...
           ;; sequence wildcard in non-final, non-prefix position → not supported
           ((sequence-wildcard-symbol-p (car pats))
            (values nil nil))
           ...
           (t
            (let* ((p1 (car pats))
                   (e1 (car exprs)))
              (multiple-value-bind (env1 success-p1)
                  (match-pattern p1 e1 env)
                (if (not success-p1)
                    (values nil nil)
                    (match-list (cdr pats) (cdr exprs) env1))))))))
